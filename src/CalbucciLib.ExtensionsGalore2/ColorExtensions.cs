...
namespace CalbucciLib.ExtensionsGalore
{
    ...
    public static class ColorExtensions
    {
        public static Color? ToColor(string? str, bool htmlSafe = false)
        {
            ...
            {
                var parts = str["rgb(".Length..].Split(new[] { ',', ')', ';' }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length != 3)
                    return null;
                int r = parts[0].ToInt();
                int g = parts[1].ToInt();
            ...
            {
                var parts = str["rgba(".Length..].Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length != 4)
                    return null;
                int r = parts[0].ToInt();
                int g = parts[1].ToInt();
            ...
            {
                var parts = str["hsl(".Length..].Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length != 3)
                    return null;
            ...
            {
                var parts = str["hsla(".Length..].Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length != 4)
                    return null;
                double h = parts[0].ToDouble() / 360.0;
                double s = parts[1].ToDouble();
            ...
            else
            {
                try
                {
                    ...
                    if (!(color.R == 0 && color.G == 0 && color.B == 0 && color.A == 0)
                        || str.Equals("black", StringComparison.InvariantCultureIgnoreCase))
                        return color;
                }
                catch (Exception)
                {
